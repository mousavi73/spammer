...
#______Quadratic_discriminant______
...
X = np.zeros([41499,7], dtype = float)
y = []
if firstLoad == 'y':

    dataset = pd.read_csv(r"C:\Users\Ratin Rayaneh\PycharmProjects\spammer\mergedData.csv" ,usecols=[0,1,4,5,6,7,8,9])
    #names = ['UserID','NumberOfFollowings','NumberOfFollowers','NumberOfTweets','LengthOfScreenName','LengthOfDescriptionInUserProfile','Bad User']
    #dataset = read_csv(dataset3, names=names)
...
X_train_1 = [0,0 , 0, 0, 0, 0,0]
X_train_2 = [0,0 , 0, 0, 0, 0,0]
...
#______Lineardiscriminant______

def Linear_discriminant(x,i,S,m_1,m_2,p_i):
    if i == 1:
        m_i = m_1
    if i == 2:
        m_i = m_2
...
row_to_be_added2=[0]
for i in range(np.size(X_validation,0)):
    result3 = Linear_discriminant(X_validation[i, :], 1, S, m_1, m_2, p_1)
    result4 = Linear_discriminant(X_validation[i, :], 2, S, m_1, m_2, p_2)
    if result3>result4:
...
row_to_be_added10=[0]
for i in range(np.size(X_test,0)):
    result3 = Linear_discriminant(X_test[i, :], 1, S, m_1, m_2, p_1)
    result4 = Linear_discriminant(X_test[i, :], 2, S, m_1, m_2, p_2)
    if result3>result4:
...
s_6=np.cov(X_train[:,5])
s_7=np.cov(X_train[:,6])
...
for i in range(np.size(X_validation,0)):
    ...
    result16 = NavieBayes(X_validation[i, 5],m_1_6,s_6)
    result17 = NavieBayes(X_validation[i, 6],m_1_7,s_7)

    final1= - 0.5 * (result11+result12+result13+result14+result15+result16+result17) + math.log(p_1)
    ...
    result116 = NavieBayes(X_validation[i, 5],m_2_6,s_6)
    result117 = NavieBayes(X_validation[i, 6],m_2_7,s_7)

    final2= - 0.5 * (result111+result112+result113+result114+result115+result116+result117) + math.log(p_2)
...
for i in range(np.size(X_validation,0)):
    ...
    result167 = Euclidean_distance(X_validation[i, 5],m_1_6)
    result177 = Euclidean_distance(X_validation[i, 6],m_1_7)

    final17= - 0.5 * (result117+result127+result137+result147+result157+result167+result177) + math.log(p_1)
    ...
    result1167 = Euclidean_distance(X_validation[i, 5],m_2_6)
    result1177 = Euclidean_distance(X_validation[i, 6],m_2_7)

    final27= - 0.5 * (result1117+result1127+result1137+result1147+result1157+result1167+result1177) + math.log(p_2)
...
for i in range(np.size(X_test,0)):
    ...
    result167 = Euclidean_distance(X_test[i, 5],m_1_6)
    result177 = Euclidean_distance(X_test[i, 6],m_1_7)
    ...
    result1167 = Euclidean_distance(X_test[i, 5],m_2_6)
    result1177 = Euclidean_distance(X_test[i, 6],m_2_7)
